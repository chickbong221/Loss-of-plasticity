...
class BasicBlock(nn.Module):
    ...
    def __init__(
        ...
        self.crelu = CReLU(inplace=True)
        self.conv2 = conv3x3(2*planes, planes)
        if has_bn:
...
class Bottleneck(nn.Module):
    ...
    def __init__(
        ...
        self.conv2 = conv3x3(width*2, width, stride, groups, dilation)
        if has_bn:
            self.bn2 = norm_layer(width)
        else:
            self.bn2 = nn.Identity()
        self.conv3 = conv1x1(width*2, planes * self.expansion)
        if has_bn:
...
def resnet8_crelu(n_obs, n_outputs, **kwargs: Any) -> ResNet: # 8 = 2 + 2 * (1 + 1 + 1)
