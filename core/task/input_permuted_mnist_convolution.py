...
class InputPermutedMNISTConvolution(Task):
    """
    ...
    def __init__(self, name="input_permuted_mnist_convolution", batch_size=1, change_freq=5000):
    ...
    def permute(self):
        ...
        self.permute_transform = torchvision.transforms.Lambda(
            lambda x: x.view(-1)[idx].view(1, 28, 28) 
        )
        self.dataset = self.get_dataset(True)
        self.iterator = self.generator()
        
if __name__ == "__main__":
    task = InputPermutedMNISTConvolution()
